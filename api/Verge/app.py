...
def create_application(id):
    ...
    name = jsonBody["name"]
    rootObject = {"id": id, "name": name, "versions": {}}
    ...
    applications = load_applications()
    applications[id] = rootObject
    store_applications(applications)

    return jsonify(rootObject), 200
...
@app.route("/api/apps/<id>/<version_id>", methods=["GET"])
def get_one_version(id, version_id):
    match = get_one_version_by_id(id, version_id)
    return jsonify(match)
